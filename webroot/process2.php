<?php
// Identical backend to process1.php.
// ModSecurity blocks malicious input before this code runs.
...
if ($result && $row = $result->fetchArray(SQLITE3_ASSOC)) {
    respond(
        'Access Granted',
        'Logged in as ' . htmlspecialchars($row['username']) . ' (' . htmlspecialchars($row['role']) . ').',
        '#3ECFA0',
        $query,
        $username
    );
...
function respond($title, $msg, $color, $query = '', $input = '') {
